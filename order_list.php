...
?>
	<form id="hiddenForm" style="display:none;" action="order_list.php" method="get">
	...
	<article class="boardArticle">
		<form id='submitForm' action="order_list.php" method="get">
		...
		<table class="total-table">
			<tbody>
				<?php
					$sql = "select COALESCE(sum(o_box),0), COALESCE(FORMAT(sum(o_delivery),0),0) from orders where DATE_FORMAT(o_date, '%Y-%m-%d') BETWEEN '".$selectedDateS."' and '".$selectedDateE."' ";
					$result = $db->query($sql);
				...
				?>
				<?php
					$sql = "select COALESCE(sum(o_box),0), COALESCE(FORMAT(sum(o_delivery),0),0) from orders where DATE_FORMAT(o_date, '%Y-%m-%d') BETWEEN '".$selectedDateS."' and '".$selectedDateE."' AND (o_type = '택배' OR o_type = '직접배송' OR o_type = '해외배송' OR o_type = '퀵' OR o_type = '이동' OR o_type = '기타배송')";
					$result = $db->query($sql);
					while($row = $result->fetch_assoc())
					{
						$datetime = explode(' ', $row['o_date']);
...
<!--						<th class="user_id" scope="col"><a href="javascript:;" class="btn_order" data-target="o_store" data-type="<?php echo $typeOfOstore;?>"  id="aOrd2">지점<?php echo $order_type_print[$typeOfOstore]; ?></a></th> -->
		...
		<h3 id="isearch">입고</h3>
		<div id="divInbound">
		<table class="intable table table-striped">
			<thead>
				<tr>
						<th class="num" scope="col">번호</th>
<!--						<th class="user_id" scope="col"><a href="javascript:;" class="btn_order" data-target="o_store" data-type="<?php echo $typeOfOstore;?>"  id="aOrd2">지점<?php echo $order_type_print[$typeOfOstore]; ?></a></th> -->
