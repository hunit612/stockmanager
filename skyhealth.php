...
?>
	<form id="hiddenForm" style="display:none;" action="skyhealth.php" method="get">
	...
	<article class="boardArticle">
		<h3>서울지점</h3>
		<form id='submitForm' action="skyhealth.php" method="get">
		...
		<table class="total-table">
			<tbody>
				<?php
					$sql = "select COALESCE(sum(o_box),0), COALESCE(FORMAT(sum(o_delivery),0),0) from orders where DATE_FORMAT(o_date, '%Y-%m-%d') BETWEEN '".$selectedDateS."' and '".$selectedDateE."' AND o_store = '본점'  ";
					$result = $db->query($sql);
				...
				?>
				<?php
					$sql = "select COALESCE(sum(o_box),0), COALESCE(FORMAT(sum(o_delivery),0),0) from orders where DATE_FORMAT(o_date, '%Y-%m-%d') BETWEEN '".$selectedDateS."' and '".$selectedDateE."' AND (o_type = '택배' OR o_type = '직접배송' OR o_type = '해외배송' OR o_type = '퀵' OR o_type = '이동' OR o_type = '기타배송')  AND o_store = '본점'";
					$result = $db->query($sql);
					while($row = $result->fetch_assoc())
					{
						$datetime = explode(' ', $row['o_date']);
