...
foreach( $name AS $key => $value ){
   ...
   $query = "insert into orders ( o_name, o_store, o_tel, o_address, o_ex2, o_type, o_date) values ('{$value}','{$store}','{$tel[$key]}','{$address[$key]}','{$ex2[$key]}','{$type[$key]}', now())";
   $db->query($query);
   $oid = $db->insert_id;
   if( !$oid ){
      $result = false;
      ...
      foreach( $code[$key] AS $inkey => $invalue ){
		 ...
		 $query =  "insert into stock  ( s_code, og_num, s_instock, s_date) values ( '{$invalue}','{$oid}','{$qty[$key][$inkey]}',now())";
		 $db->query($query);
