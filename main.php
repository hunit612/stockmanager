...
<body>
	...
	<!--
			<form id='submitForm' action="main.php" method="get">
				<!--
				...
				?>
			   <input name="selectedDateS" id="datepicker" value="<?php echo $selectedDateS;?>"/> ~ 
			   <input name="selectedDateE" id="datepicker2" value="<?php echo $selectedDateE;?>"/>

			   <button type="button" id="btnSearch" class="btn btn-default">조회</button>
			</form>
			-->
		<div class="table-top">
				<button type="button" onclick="location.href='main_search.php'" class="searchBtn2 btn btn-default">기간별 조회</button>
		<form action="product_search.php" class="searchM2" method="post">
		...
		<table class="stock-table table table-striped table-hover">
			...
			<tbody>
				<?php
					$total_array = array();
					$sql = 'select p.p_code, sum(s.s_instock + s.s_return), sum(s.s_outstock)  from product as p, stock as s WHERE p.p_code = s.s_code group by p.p_code';
					$result = $db->query($sql);
					while($row = $result->fetch_assoc()){
						$total_array[$row['p_code']] = $row['sum(s.s_instock + s.s_return)'] - $row['sum(s.s_outstock)'];
					}
					$sql = 'select p.p_code, p.p_name, sum(s.s_instock + s.s_return), sum(s.s_outstock) from product as p, stock as s where p.p_code = s.s_code group by p.p_code';
					$result = $db->query($sql);
						while($row = $result->fetch_assoc())
						{
